...
function parseFilter(raw, previous) {
...
program
    ...
    .option('--input-format <csv|xml>', 'Force input format. Auto-detected from file extension or URL if omitted.', 'auto')
    .option('-f, --filter <column=value>', 'Filter rows by supplier column value (repeatable, case-insensitive). Example: --filter "BRAND=Carrera"', parseFilter, [])
    .option('-v, --verbose', 'Enable debug logging', false)
    .action(async (opts) => {
    if (!opts.input) {
        console.error('error: required option \'-i, --input <path|url>\' not specified');
        process.exit(1);
    }
    if (!opts.output) {
    ...
    if (!opts.mapping) {
        console.error('error: required option \'-m, --mapping <path>\' not specified');
        process.exit(1);
    }
