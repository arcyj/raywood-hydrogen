...
const DEFAULT_USER_AGENT = 'Mozilla/5.0 (compatible; shopify-importer/1.0; +https://github.com/shopify-importer)';
let _client = null;
function getClient() {
    if (!_client) {
 ...
 */
export async function scrapeUrl(url, config, retries = 2) {
    const result = { fields: {}, errors: [] };
    let html;
    try {
        const response = await withRetry(() => getClient().get(url, { responseType: 'text' }), retries);
        html = response.data;
    }
    catch (err) {
