...
import { z } from 'zod';
import type { MappingConfig } from '../types/index.js';
...
export async function loadMappingConfig(filePath: string): Promise<MappingConfig> {
  ...
  try {
    ...
    throw new ConfigValidationError(
      `Failed to read mapping file: ${filePath}`,
  ...
  try {
    ...
    throw new ConfigValidationError(
      `Mapping file is not valid JSON: ${filePath}`,
      [err instanceof Error ? err.message : String(err)],
    );
  }
  ...
  if (!result.success) {
    ...
    throw new ConfigValidationError(
      `Mapping config validation failed`,
      issues,
    );
  }
