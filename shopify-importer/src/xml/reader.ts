 ...
 */
async function fetchXmlContent(source: string): Promise<string> {
 ...
 */
function findElements(obj: unknown, elementName: string): unknown[] {
 ...
 */
function valueToString(val: unknown, join?: string): string {
  if (val === null || val === undefined) return '';

  if (Array.isArray(val)) {
    const strings = (val as unknown[]).map((v) => valueToString(v)).filter(Boolean);
 ...
 */
function itemToRow(item: unknown, config: XmlReaderConfig): SupplierRow {
 ...
 */
export async function* createXmlReader(
  source: string,
  ...
  const parsed: unknown = parser.parse(xmlContent);
