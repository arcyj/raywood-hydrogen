...
function rowMatchesFilters(row: SupplierRow, filters: RowFilter[]): boolean {
...
function resolveInputFormat(inputPath: string, explicit: InputFormat): 'csv' | 'xml' {
  if (explicit === 'csv' || explicit === 'xml') return explicit;
  if (/\.xml$/i.test(inputPath)) return 'xml';
  if (inputPath.startsWith('http://') || inputPath.startsWith('https://')) return 'xml';
  return 'csv';
}
...
export async function runPipeline(options: PipelineOptions): Promise<PipelineResult> {
  ...
  if (format === 'xml' && !config.xmlReader) {
    throw new Error(
      `Input format is XML but mapping config is missing an "xmlReader" block. ` +
  ...
  const result: PipelineResult = { processed: 0, failed: 0, skipped: 0, mergeMatched: 0, mergeMissed: 0 };
  const inFlight: Promise<void>[] = [];

  async function processRow(row: SupplierRow, rowIndex: number): Promise<void> {
    try {
      let enriched = row;
      ...
      let scraped: Partial<ShopifyRow> = {};
  ...
  const rowSource: AsyncIterable<SupplierRow> =
    format === 'xml'
