...
export function useScaleBackground() {
  ...
  const initialBackgroundColor = React.useRef<string | null>(null);

  React.useEffect(() => {
    initialBackgroundColor.current = document.body.style.backgroundColor;
  }, []);
    ...
    if (isOpen && shouldScaleBackground) {
      ...
      return () => {
        ...
        timeoutIdRef.current = window.setTimeout(() => {
          if (initialBackgroundColor.current) {
            document.body.style.background = initialBackgroundColor.current;
          } else {
